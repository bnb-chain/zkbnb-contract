...
contract ZkBNBTest is ZkBNB {
  ...
  function mintNFT(address _creatorAddress, address _toAddress, uint256 _nftTokenId, bytes memory _extraData) external {
    return INFTFactory(defaultNFTFactory).mintFromZkBNB(_creatorAddress, _toAddress, _nftTokenId, _extraData);
  }
}
