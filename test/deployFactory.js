...
describe('ZkBNB contract', function () {
  ...
  describe('ZkBNB Deploy Test', function () {
    it('test ZNS register', async function () {
      ...
      const registerZNSTx = await zkbnbProxy
        .connect(addr1)
        .registerZNS('sher', await addr1.getAddress(), sherPubKey);
      ...
      expect(
        await zkbnbProxy
          .connect(owner)
          .getAddressByAccountNameHash(sherNameHash),
      ).to.equal(await addr1.getAddress());
      expect(await znsRegistry.owner(sherNameHash)).to.equal(
        await addr1.getAddress(),
      );

      // check price oracle
      const sherfromzkbnbPubKey = ethers.utils.formatBytes32String(
        'sherfromzkbnb.legend',
      ); // need 1 bnb for fee
      await expect(
        zkbnbProxy.registerZNS(
          'sherfromzkbnb',
          await addr1.getAddress(),
          sherfromzkbnbPubKey,
        ),
    ...
    it('test Deposit BNB', async function () {
      // register ZNS
      const sherPubKey = ethers.utils.formatBytes32String('sher.legend');
      const registerZNSTx = await zkbnbProxy.registerZNS(
        'sher',
        await addr1.getAddress(),
        sherPubKey,
      );
      ...
      const depositBNBTx = await zkbnbProxy
        .connect(addr1)
        .depositBNB(sherNameHash, {
    ...
    it('test Deposit BEP20', async function () {
      ...
      // set allowance
      const setAllowanceTx = await token
        .connect(addr1)
        .approve(zkbnbProxy.address, 10000);
      await setAllowanceTx.wait();
      expect(await token.allowance(addr1.address, zkbnbProxy.address)).to.equal(
        10000,
      );

      // add asset
      const addAssetTx = await assetGovernance
        .connect(owner)
        .addAsset(token.address);
      ...
      const depositBEP20Tx = await zkbnbProxy
        .connect(addr1)
        .depositBEP20(token.address, 100, sherNameHash);
    ...
    it('test Deposit ERC721', async function () {
      ...
      const sherNameHash = namehash.hash('sher.legend');
      const depositNftTx = await zkbnbProxy.depositNft(
        sherNameHash,
        erc721.address,
        '0',
      );
    ...
    it('test RequestFullExit', async function () {
      ...
      const requestFullExitTx = await zkbnbProxy
        .connect(addr1)
        .requestFullExit(sherNameHash, token.address);
