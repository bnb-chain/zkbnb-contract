...
describe("Zkbas-Legend contract", function () {
    ...
    let zkbasProxy, bankProxy, gatekeeper;

    let abi1 = require('../artifacts/contracts/test-contracts/ZkbasUpgradeTest.sol/ZkbasUpgradeTest.json').abi
    ...
    beforeEach(async function () {
        ...
        // deploy zkbas
        Zkbas = await ethers.getContractFactory("ZkbasUpgradeTest");
        ...
        factory = await DeployFactory.connect(owner).deploy(zkbas.address, bank.address);

        // get deployed proxy contract and the gatekeeper contract
        let tx = await factory.deployTransaction;
        let receipt = await tx.wait();
        const AddressesInterface = new ethers.utils.Interface(["event Addresses(address zkbas, address bank, address gatekeeper)"]);
        ...
        zkbasProxy = new ethers.Contract(event[0], abi1, provider)
        bankProxy = new ethers.Contract(event[1], abi2, provider)
        gatekeeper = new ethers.Contract(event[2], abi3, provider)
    });

    describe('Zkbas Upgrade Test', function () {
        it("test normal upgrade", async function () {
            // before upgrade: balance = 0
            let tx1 = await zkbasProxy.connect(addr1).setBalance(5)
            await tx1.wait()
            let tx2 = await zkbasProxy.connect(addr1).setBalance(5)
            await tx2.wait()
            // expect balance = 10
            expect(await zkbasProxy.connect(addr1).balance()).to.equal(10);
            ...
            // deploy new zkbas contract
            Zkbas2 = await ethers.getContractFactory("ZkbasUpgradeTargetTest");
            zkbas2 = await Zkbas2.deploy()
            await zkbas2.deployed()

            // --- main upgrade workflow ---
            // start upgrade
            let newTarget = [zkbas2.address, ethers.constants.AddressZero]
            ...
            expect(await zkbasProxy.connect(addr1).balance()).to.equal(22);

            // after upgrade
            let tx8 = await zkbasProxy.connect(addr1).setBalance(10)
            await tx1.wait()
            // expect balance = 72 = 22 + 10 * 5
            expect(await zkbasProxy.connect(addr1).balance()).to.equal(72);
        });
    });
});
