...
async function main() {
  ...
  let setAllowanceTx = await BUSDToken.approve(
    zkbnb.address,
    ethers.utils.parseEther('100000000000'),
  );
  await setAllowanceTx.wait();

  setAllowanceTx = await LEGToken.approve(
    zkbnb.address,
    ethers.utils.parseEther('100000000000'),
  );
  await setAllowanceTx.wait();
  setAllowanceTx = await REYToken.approve(
    zkbnb.address,
    ethers.utils.parseEther('100000000000'),
  );
  await setAllowanceTx.wait();
  ...
  depositBEP20 = await zkbnb.depositBEP20(
    LEGToken.address,
    ethers.utils.parseEther('100'),
    'sher',
  );
